...
int	farm_routine(t_base *base, t_entity *entity, t_hero *hero)
{
	...
	{
		target = get_target_closest_from_base(base, entity);
		if (target == -1 || !is_in_range(entity[target].x, entity[target].y, hero->x, hero->y, MOVE_RADIUS))
...
int	patrol_routine(t_base *base, t_entity *entity, t_hero *hero)
{
	...
	for (int i = 0; i < base->entity_count; i++)
	{
		...
		if (entity[i].type == 0)
		{
			...
			{
				if (is_in_range(entity[i].x, entity[i].y, hero->x, hero->y, WIND_RADIUS) && base->mana >= 10)
					return (spell_wind(base, base->ex, base->ey, "OUAF"));
		...
		if (entity[i].type == 2)
		{
			...
			else if (is_in_range(entity[i].x, entity[i].y, hero->x, hero->y, MOVE_RADIUS) && base->mana >= 10)
	...
	else
	{
		...
		for (int i = 0; i < base->entity_count; i++)
		{
			if (entity[i].type == 0)
			{
				...
				else if (entity[i].shield_life == 0 && is_in_range(entity[i].x, entity[i].y, base->x, base->y, PROBLEM_RADIUS) && is_in_range(entity[i].x, entity[i].y, hero->x, hero->y, WIND_RADIUS) && base->mana >= 10)
					return (spell_wind(base, base->ex, base->ey, "AAAAA"));
				else if (is_in_range(entity[i].x, entity[i].y, base->x, base->y, PROBLEM_RADIUS))
					return (move_to_target(entity[i], "GRRR"));
				else if (is_in_range(entity[i].x, entity[i].y, base->x, base->y, MENACE_RADIUS) && entity[i].threat_for == 1)
					return (move_to_target(entity[i], "GRRR"));
			}
		}
	}
